"""Shared utilities used by both the legacy and routed runner paths."""
...
class RoutedAnalysisResult:
    """Bundle the routing-layer artifacts with an optional legacy report.

    When scoped execution handles the full module set, ``report`` is None —
    the legacy engine never ran.  Consumers that need a full
    ``AnalysisReport`` should call the lazy-loading helpers in
    ``briarwood.dash_app.data`` to trigger the legacy engine on demand.
    """
