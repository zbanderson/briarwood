...
def run_margin_sensitivity(context: ExecutionContext) -> dict[str, object]:
    ...
    arv_output = _require_prior_output(context, "arv_model")
    renovation_output = _require_prior_output(context, "renovation_impact")
    carry_output = _require_prior_output(context, "carry_cost")
    ...
    monthly_carry = float(carry_metrics.get("total_monthly_cost") or carry_metrics.get("gross_monthly_cost") or 0.0)
...
def _require_prior_output(context: ExecutionContext, module_name: str) -> dict[str, Any]:
    ...
    if not isinstance(output, dict):
        raise ValueError(
            f"Module '{module_name}' must run before margin_sensitivity so its scoped output is available."
        )
    return output
