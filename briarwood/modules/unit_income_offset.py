...
def run_unit_income_offset(context: ExecutionContext) -> dict[str, object]:
    ...
    Error contract (DECISIONS.md 2026-04-24): internal exceptions return
    a ``module_payload_from_error`` fallback. The carry-cost prior is
    optional; absence is handled in-place by ``_carry_cost_metric``.
    """
    ...
    try:
        ...
        return module_payload_from_error(
            module_name="unit_income_offset",
            context=context,
            summary="Unit income offset unavailable — internal failure composing the accessory-unit snapshot.",
            warnings=[f"Unit-income-offset fallback: {type(exc).__name__}: {exc}"],
