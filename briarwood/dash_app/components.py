    ...
    cards = [
        html.Div(
            [
                ...
                html.Div(ls.risk_narrative[:60], style={"fontSize": "11px", "color": TEXT_SECONDARY, "lineHeight": "1.4"}),
            ],
                ...
                [
                    html.Summary(
                        [
                            html.Div(
                                [
                                    html.Span(cat.category_name, style={"fontSize": "11px", "color": TEXT_MUTED, "minWidth": "100px", "display": "inline-block"}),
                    ...
                    html.Div(
                        [
                            html.Div(
                                [
                                    html.Span("What this means", style={"fontSize": "11px", "fontWeight": "600", "color": TEXT_MUTED, "textTransform": "uppercase"}),
                                    html.Div(
...
def _render_category_components(cat: object) -> html.Div | None:
        ...
        rows.append(
            html.Div(
                [
                    html.Div(
                        [
                            html.Span(label_map.get(key, key.replace("_", " ").title()), style={"fontSize": "11px", "color": TEXT_MUTED}),
                            html.Span(f"{value:.1f}/5", style={"fontSize": "11px", "fontWeight": "600", "color": sc}),
                        ],
                        style={"display": "flex", "justifyContent": "space-between", "marginBottom": "2px"},
                    ),
                    html.Div(
                    ...
                    html.Div(component_notes.get(key, ""), style={"fontSize": "11px", "lineHeight": "1.4", "color": TEXT_MUTED}),
                ],
    ...
    return html.Div(
        [
            html.Div("Sub-Components", style={"fontSize": "11px", "fontWeight": "600", "color": TEXT_MUTED, "textTransform": "uppercase", "marginBottom": "4px"}),
        ...
        rows.append(
            html.Div(
                [
                    ...
                    html.Span(f"({sf.weight:.0%})", style={"fontSize": "11px", "color": TEXT_MUTED, "minWidth": "35px", "display": "inline-block"}),
                    html.Span(sf.evidence, style={"fontSize": "11px", "color": TEXT_MUTED, "flex": "1"}),
                ],
...
def render_executive_summary(view: PropertyAnalysisView) -> html.Div:
    ...
    metric_rows = [
        ...
        ("Fair Value", _fmt_value(view.bcv), None),
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                ...
                style={"fontSize": "13px", "lineHeight": "1.6", "color": TEXT_PRIMARY, "marginBottom": "12px"},
            ),
            # Strengths vs Risks
            html.Div(
                [
                            ...
                            [
                                html.Tr(
                                    [
                                        ...
                                        html.Td(
                                            change or "",
                                            style={"padding": "3px 0 3px 6px", "fontSize": "11px", "textAlign": "right", "borderBottom": f"1px solid {BORDER_SUBTLE}",
        ...
        html.Div(
            [
                ...
                html.Span(
                    ...
                    style={"fontSize": "11px", "color": TEXT_MUTED, "marginLeft": "6px"},
                ),
    ...
    if cat:
        ...
        body_children.append(
            html.Details(
                [
                    html.Summary(
                        html.Span("Sub-Factor Breakdown", style={"fontSize": "11px", "fontWeight": "600", "color": TEXT_MUTED, "textTransform": "uppercase", "cursor": "pointer"}),
...
def _verdict_bullet(icon: str, text: str, color: str) -> html.Div:
    return html.Div(
        [
            html.Span(icon, style={"color": color, "marginRight": "8px", "fontSize": "14px", "lineHeight": "1.5"}),
            html.Div(text, style={"fontSize": "13px", "lineHeight": "1.5", "flex": "1", "color": TEXT_PRIMARY}),
        ],
        style={"display": "flex", "alignItems": "flex-start", "marginBottom": "6px"},
    )


def _generate_bottom_line(view: PropertyAnalysisView) -> str:
    """Generate a one-sentence bottom line from lens + score data."""
    ls = view.lens_scores
    best = ls.recommended_lens if ls else ""
    score = view.final_score or 3.0
    if best == "owner":
        if score >= 4.0:
            return "The location and trajectory support the price — this makes sense to live in."
        if score >= 3.0:
            return "Livable, with some compromises on condition or carry. Worth a closer look."
        return "The lifestyle case is thin at this price — compare alternatives."
    if best == "investor":
        if score >= 4.0:
            return "The cash flow math works — this property carries itself as a rental."
        if score >= 3.0:
            return "Viable rental with acceptable returns, but not a standout."
        return "The numbers don't support a rental hold at this price."
    if best == "developer":
        if score >= 4.0:
            return "The property can physically become more — renovation or expansion pencils."
        if score >= 3.0:
            return "Some development upside exists, but it's selective, not commanding."
        return "Limited value-add potential — better opportunities likely exist."
    if score >= 4.0:
        return "The thesis is supported across multiple buyer profiles."
    if score >= 3.0:
        return "More right than wrong — but the primary condition needs to hold."
    return "Not enough signal to act — resolve the top gap before taking a position."
...
def render_property_verdict(view: PropertyAnalysisView) -> html.Div:
    """Property Verdict Card — answers 'Should I buy this?' in 5 seconds."""
    if view.final_score is None:
        return html.Div()
    ...
    lens_name, _icon = _LENS_DISPLAY.get(best_key, (best_key.replace("_", " ").title(), "📊"))
        ...
        children=[
            ...
            html.Div(
                [
                    html.Div(
                        [
                            html.Div("STRENGTHS", style={"fontSize": "11px", "fontWeight": "600", "color": ACCENT_GREEN, "marginBottom": "8px", "letterSpacing": "0.5px"}),
                            html.Div([_verdict_bullet("✓", s, ACCENT_GREEN) for s in strengths]),
                        ],
                        style={"flex": "1"},
                    ),
                    html.Div(
                        [
                            html.Div("RISKS", style={"fontSize": "11px", "fontWeight": "600", "color": ACCENT_ORANGE, "marginBottom": "8px", "letterSpacing": "0.5px"}),
                            html.Div([_verdict_bullet("⚠", r, ACCENT_ORANGE) for r in risks]),
                        ],
                        style={"flex": "1"},
                    ),
                ],
                style={"display": "flex", "gap": "24px", "marginBottom": "16px"},
            ) if (strengths or risks) else None,
            # Bottom line
            html.Div(
                [
                    html.Div("BOTTOM LINE", style={"fontSize": "11px", "fontWeight": "600", "color": TEXT_MUTED, "marginBottom": "6px", "letterSpacing": "0.5px"}),
                    html.Div(bottom_line, style={"fontSize": "13px", "lineHeight": "1.6", "fontStyle": "italic", "color": TEXT_PRIMARY}),
                ],
...
def render_perspective_block(view: PropertyAnalysisView) -> html.Div:
    """Supporting buyer-fit block — auto-picks the clearest lens and shows alternatives."""
    ls = view.lens_scores
    if ls is None:
        return html.Div()

    best_key = ls.recommended_lens if ls.recommended_lens in {"risk", "investor", "owner", "developer"} else "owner"
    ...
    other_badges = []
    if best_key != "owner" and ls.owner_score is not None:
        other_badges.append(_compact_lens_badge("🏠 Owner", ls.owner_score))
    if best_key != "investor" and ls.investor_score is not None:
        other_badges.append(_compact_lens_badge("💰 Investor", ls.investor_score))
    if best_key != "developer" and ls.developer_score is not None:
        other_badges.append(_compact_lens_badge("🔧 Developer", ls.developer_score))
    other_badges.append(_compact_lens_badge("🛡 Risk", ls.risk_score, inverted=True))
    ...
    return html.Div(
        [
            html.Div("Buyer Fit Perspective", style={**SECTION_HEADER_STYLE, "marginBottom": "8px"}),
            html.Div(
                "Briarwood uses the lens model as supporting interpretation, not a primary control. The page now auto-highlights the buyer type this property fits best and keeps the other perspectives visible for context.",
                style={"fontSize": "12px", "lineHeight": "1.6", "color": TEXT_SECONDARY, "marginBottom": "12px"},
            ),
            html.Div(
                [
                    html.Div(
                        ...
                        style={"fontSize": "14px", "fontWeight": "600", "color": TEXT_PRIMARY},
                            ...
                            html.Div(
                                [
                                    html.Span(f"{best_score:.1f}", style={"fontSize": "24px", "fontWeight": "700", "color": sc}),
                                    html.Span(" / 5", style={"fontSize": "14px", "color": TEXT_MUTED}),
                                    html.Span(f" — {sl}", style={"fontSize": "13px", "fontWeight": "600", "color": sc, "marginLeft": "8px"}),
                                ],
                            ),
                        ],
                        style={"display": "flex", "justifyContent": "space-between", "alignItems": "center", "marginBottom": "12px"},
                    ),
                    html.Div(best_narrative, style={"fontSize": "13px", "lineHeight": "1.6", "color": TEXT_PRIMARY, "marginBottom": "12px"}) if best_narrative else None,
                ],
                style={**CARD_STYLE, "border": f"2px solid {sc}40", "padding": "16px 20px", "marginBottom": "12px"},
            ),
            html.Div(
                [b for b in other_badges if b is not None],
                style={"display": "flex", "gap": "8px", "flexWrap": "wrap", "marginBottom": "16px"},
            ),
            # Category breakdown bars
            html.Div(
                [
                    html.Div("CATEGORY BREAKDOWN", style={**SECTION_HEADER_STYLE, "marginBottom": "8px"}),
                    cat_bars,
                ],
            ),
        ],
...
def _risk_answer(view: PropertyAnalysisView) -> tuple[str, list[str]]:
    ...
    top_risks = view.top_risks[:3] or ([view.biggest_risk] if view.biggest_risk else [])
    if len(risk_bits) == 1:
...
def _realtor_talking_points(view: PropertyAnalysisView, report: AnalysisReport) -> list[str]:
    ...
    for item in [price_answer, forward_answer, optionality_answer]:
        if item and "limited" not in item.lower() and item not in points:
            points.append(item)
    for reason in (view.top_reasons or []):
...
def _realtor_watchouts(view: PropertyAnalysisView) -> list[str]:
    ...
    for dep in (view.what_changes_call or [])[:2]:
...
def _value_snapshot_top_section(view: PropertyAnalysisView, report: AnalysisReport) -> html.Div:
    ...
    trend_box = html.Div(
        [
            html.Div("Value Context", style={**LABEL_STYLE, "marginBottom": "6px"}),
            html.Div(
                "This pairs today's value anchor with the current forward range. A longer historical trend can slot in here when property or market history is available in the analysis view.",
                style={"fontSize": "12px", "lineHeight": "1.55", "color": TEXT_SECONDARY, "marginBottom": "8px"},
            ),
            forward_fan_chart(view, compact=True),
        ],
        style={**CARD_STYLE, "padding": "14px 16px"},
    )
    ...
    return _property_analysis_section(
        ...
        "Answer the current value question first, then frame the immediate upside or downside around the asking price.",
        [
            html.Div(
                [value_box, trend_box],
                style={"display": "grid", "gridTemplateColumns": "repeat(auto-fit, minmax(300px, 1fr))", "gap": "12px", "alignItems": "start"},
            ),
            html.Div(comp_positioning_dot_plot(view, report), style={"marginTop": "10px"}),
        ],
    )
...
def _rent_ramp_break_even_section(view: PropertyAnalysisView, report: AnalysisReport) -> html.Div:
    ...
    if not isinstance(monthly_rent, (int, float)):
        return _property_analysis_section(
            "Section D - Financial Path / Rent Ramp",
    ...
    return html.Div(
        [
            ...
            inline_metric_strip([
                ...
                ("Memo Verdict", view.memo_verdict or "—", None),
                ("Confidence", f"{view.overall_confidence:.0%}", None),
            ]),
            html.Div(
                [
                    _list_block("Why It Fits", view.buyer_fit[:3], tone="positive"),
                    _list_block("What Changes the Call", view.what_changes_call[:3], tone="warning"),
                ],
...
def render_tear_sheet_body(
    ...
    *,
    ...
    # Property-driven smart defaults: determine which sections to auto-expand
    ...
    risk_section = _question_section(
        "What Could Break the Thesis?" if selected_mode == "owner" else "Risks / Things To Be Aware Of",
        risk_answer if selected_mode == "owner" else _reframe_risk_for_realtor(risk_answer),
        ...
        extra_content=html.Div(
            [block for block in [
                ...
                _list_block("Client-Facing Reframes", [_reframe_risk_for_realtor(item) for item in top_risks[:3]], tone="warning") if selected_mode == "realtor" else None,
            ] if block is not None],
            style={"display": "grid", "gap": "8px"},
        ),
        ...
        [
            ...
            html.Div(
                [
                    ...
                    _list_block("Watch-Outs", [_reframe_risk_for_realtor(item) for item in view.risk_location.risks[:4]], tone="warning"),
                ],
    ...
    realtor_economics_support = html.Div(
        [
            html.Div("Ownership Economics", style=SECTION_HEADER_STYLE),
            html.Div(
                [
                    income_carry_waterfall(view, report),
                    _economics_summary_box(view, report),
    ...
    evidence_section = _question_section(
        "How Strong Is the Evidence?" if selected_mode == "owner" else "Evidence / Confidence",
        f"Overall confidence is {view.overall_confidence:.0%}, with the analysis strongest where facts and user-confirmed inputs replace inferred assumptions." if selected_mode == "owner" else f"Overall confidence is {view.overall_confidence:.0%}. Use this section to separate what is confirmed from what still needs validation with a client or during diligence.",
        section_id="tear-evidence",
        confidence=_section_confidence(view, "evidence"),
        default_open=False,
        summary="This section distinguishes sourced facts, user-confirmed inputs, estimated assumptions, and unresolved gaps so the Briarwood conclusion can be read in context." if selected_mode == "owner" else "This is the trust layer behind the positioning: what is known, what is estimated, and what would most improve confidence.",
        metrics_strip=_evidence_summary_strip(view),
        basis_chips=_section_status_chips(
            view,
            ["price_to_rent", "net_monthly_cost", "capex_load", "liquidity", "market_momentum", "net_opportunity_delta"],
        ),
        extra_content=html.Div(
            [
        ...
        missing_note=_section_missing_note(
            view,
            ["price_to_rent", "net_monthly_cost", "capex_load", "liquidity", "optionality", "net_opportunity_delta", "market_momentum"],
        ),
    )
    ...
    realtor_fit_section = _question_section(
        ...
        confidence=_section_confidence(view, "optionality"),
        default_open=True,
        summary="Use this section to align the property with the buyer type most likely to understand and accept the tradeoffs.",
        metrics_strip=inline_metric_strip([
            ("Best Fit", view.decision.best_fit if view.decision is not None else "—", None),
            ("Positioning", _soft_recommendation_label(view.decision.recommendation) if view.decision is not None else "—", None),
            ("Confidence", f"{view.overall_confidence:.0%}", None),
            ("Optionality", optionality_answer, None),
        ]),
        extra_content=render_perspective_block(view),
    )
    realtor_why_section = _question_section(
        "Why This Works",
        f"{price_answer} {forward_answer}",
        section_id="tear-why",
        confidence=min(_section_confidence(view, "price"), _section_confidence(view, "forward")),
        default_open=True,
        summary="This combines current value support, forward scenarios, and the broader market setup into the core positioning case.",
        metrics_strip=inline_metric_strip([
            ("Ask", _fmt_compact(view.ask_price), None),
            ("Fair Value", _fmt_compact(view.bcv), gap_pct_text(view) if view.mispricing_pct is not None else None),
            ("Base", _fmt_compact(view.base_case), None),
        ...
        chart=html.Div(
            [
                comp_positioning_dot_plot(view, report),
                html.Div(forward_fan_chart(view), style={"marginTop": "8px"}),
            ],
            style={"display": "grid", "gap": "8px"},
        ),
    )
    realtor_talking_points_section = _question_section(
        "Buyer Talking Points / Defensible Positives",
        "Lead with the parts of the story that are both real and supportable.",
        section_id="tear-talking-points",
        confidence=_section_confidence(view, "price"),
        default_open=False,
        summary="These points come directly from Briarwood's current valuation, forward view, optionality, and local-market drivers.",
        extra_content=html.Div(
            [block for block in [
                _list_block("Talking Points", _realtor_talking_points(view, report), tone="positive"),
                _optionality_fact_block(view),
                realtor_market_support,
            ] if block is not None],
            style={"display": "grid", "gap": "8px"},
        ),
    )
    realtor_objections_section = _question_section(
        "What Buyers Will Ask",
        "Expect questions around condition, carrying cost, and how durable the upside case really is.",
        section_id="tear-objections",
        confidence=_section_confidence(view, "risk"),
        default_open=False,
        summary="This section translates the main underwriting objections into practical buyer questions an agent should be ready for.",
        extra_content=html.Div(
            [block for block in [
                _list_block("Likely Buyer Questions", [_reframe_risk_for_realtor(item) for item in top_risks[:3]], tone="warning"),
                realtor_economics_support,
            ] if block is not None],
            style={"display": "grid", "gap": "8px"},
        ),
    )
    ...
    realtor_tabs = [
        ...
        (
            ...
            [
                realtor_fit_section,
                realtor_talking_points_section,
                realtor_objections_section,
            ],
        ),
        (
            "renovation",
        ...
        (
            ...
            [
                evidence_section,
                market_section,
                _realtor_positioning_close(view),
            ],
        ),
    ]
...
_TOUR_STEPS: list[dict[str, str | list[str]]] = [
    {
        "title": "Welcome to Briarwood",
        "lines": [
            "Briarwood is a real estate intelligence platform that helps you decide",
            "whether a property is worth pursuing — and why.",
            "",
            "This quick tour shows you how to read the report.",
    ...
    },
    {
        ...
        "lines": [
            "Every report opens with a clear call:",
            "Buy → Neutral → Avoid.",
            "",
            "You'll see the overall score, top strengths and risks,",
            "and a one-sentence bottom line.",
            "This is THE answer — everything below is the evidence behind it.",
        ],
    },
    {
        "title": "How to Read the Report",
        "lines": [
            "The report is structured as a narrative — read it top to bottom:",
            "",
            "  1. Is the Price Right? — Am I overpaying?",
            "  2. What Does the Forward Look Like? — Where does value go from here?",
            "  3. Where's the Upside? — Can this property become more?",
            "  4. What Does It Cost to Own? — What's the monthly reality?",
            "  5. What Could Break the Thesis? — What kills the deal?",
            "",
            "Each section answers one question. If you only have 60 seconds,",
            "read the bold answer line at the top of each section.",
        ],
    },
    {
        "title": "Scores and Confidence",
        "lines": [
            "Every score is on a 1–5 scale with a quality label:",
            "  Clear (4.5+) — the data strongly supports the score",
            "  Supported (4.0+) — solid evidence, minor gaps",
            "  Mixed (3.0+) — some evidence both ways",
            "  Thin (2.0+) — limited data, treat with caution",
    ...
    },
    {
        "title": "Four Lenses",
        "lines": [
            "The same property can be compelling for one buyer and wrong for another.",
            "Briarwood evaluates through 4 lenses:",
            "",
            "  Risk Assessment — How safe is this purchase?",
            "  Investor — Does the cash flow math work?",
            "  Owner-Occupant — Does it make sense to live in?",
            "  Developer — Can you build or renovate into upside?",
            "",
            "The 'Best For' badge shows which lens fits the property.",
            "You can switch lenses to see the same data scored differently.",
        ],
    },
    {
        "title": "Key Metrics — Renamed for Clarity",
        "lines": [
            "A few labels you'll see throughout the report:",
            "",
            "  Fair Value — Briarwood's estimate of current worth",
            "  Rent Coverage — how much rent offsets the carry",
    ...
    {
        "title": "Expanding the Details",
        "lines": [
            "Each section is collapsible. Click to expand and you'll find:",
            "  • Charts and visualizations for the underlying data",
            "  • Sub-factors — the specific drivers behind the section score",
            "  • Insight callouts — highlighted takeaways in context",
            "",
            "Sub-factors are phrased as questions (e.g., 'Priced right vs. recent sales?').",
            "The top drivers auto-expand; click 'Show all' for the full breakdown.",
        ],
    },
    {
        "title": "You're Ready",
        "lines": [
            "To recap — the fastest way to read a Briarwood report:",
            "",
            "  1. Read the verdict (top of page) — is this a Buy?",
            "  2. Check 'Is the Price Right?' — am I getting a fair deal?",
            "  3. Check the Forward — does value go up or down from here?",
            "  4. Check the Upside — can I create more than what's priced in?",
            "  5. Skim Cost to Own and Risks — any deal-breakers?",
            "",
            "Use the What-If slider to test scenarios,",
            "or Compare to evaluate properties side-by-side.",
...
def render_tour_overlay(step: int = 0) -> html.Div:
    """Render the guided tour overlay for first-time users."""
    total = len(_TOUR_STEPS)
    step = max(0, min(step, total - 1))
    data = _TOUR_STEPS[step]

    progress = [
        html.Div(style={
            "width": "40px", "height": "4px", "borderRadius": "2px",
            "backgroundColor": ACCENT_BLUE if i <= step else BORDER,
        })
        for i in range(total)
    ]

    body_lines = [
        html.P(line, style={"fontSize": "14px", "lineHeight": "1.6", "marginBottom": "6px", "color": TEXT_PRIMARY})
        for line in data["lines"]
    ]

    prev_label = "Skip Tour" if step == 0 else "Previous"
    next_label = "Finish" if step == total - 1 else "Next"

    return html.Div(
        html.Div(
            [
                # Progress dots
                html.Div(progress, style={"display": "flex", "gap": "8px", "justifyContent": "center", "marginBottom": "24px"}),
                # Title
                html.Div(data["title"], style={"fontSize": "24px", "fontWeight": "700", "color": TEXT_PRIMARY, "marginBottom": "16px"}),
                # Body
                html.Div(body_lines),
                # Nav
                html.Div(
                    [
                        html.Button(prev_label, id="tour-prev-btn", n_clicks=0, style={**BTN_SECONDARY, "padding": "8px 16px"}),
                        html.Div(f"Step {step + 1} of {total}", style={"fontSize": "13px", "color": TEXT_MUTED, "alignSelf": "center"}),
                        html.Button(next_label, id="tour-next-btn", n_clicks=0, style={**BTN_PRIMARY, "padding": "8px 16px"}),
                    ],
                    style={"display": "flex", "justifyContent": "space-between", "marginTop": "24px"},
                ),
            ],
            style={
                "backgroundColor": BG_SURFACE, "border": f"2px solid {BORDER}",
                "borderRadius": "8px", "padding": "32px", "maxWidth": "600px", "width": "100%",
            },
        ),
        id="tour-overlay",
        style={
...
def render_tour_trigger_button() -> html.Div:
    """Floating button to re-launch the tour."""
    return html.Div(
        html.Button(
            "? Tour", id="tour-trigger-btn", n_clicks=0,
            style={
                "padding": "10px 16px", "backgroundColor": ACCENT_BLUE,
                "border": "none", "borderRadius": "20px", "color": BG_BASE,
...
def render_what_if_slider(view: PropertyAnalysisView) -> html.Div:
    ...
    min_ask = int(ask * 0.80 / 5000) * 5000
    ...
    return html.Div(
        ...
        style={
            **CARD_STYLE, "borderLeft": f"4px solid {ACCENT_BLUE}",
...
def render_portfolio_dashboard(views: list[PropertyAnalysisView]) -> html.Div:
    ...
    # Category heatmap (using existing component)
    heatmap = score_comparison_heatmap(scored) if len(scored) >= 2 else html.Div("Need 2+ scored properties for heatmap.", style={"color": TEXT_MUTED, "fontSize": "13px"})

    heatmap_section = html.Div(
        [
            html.Div("CATEGORY SCORES HEATMAP", style=SECTION_HEADER_STYLE),
            heatmap,
        ],
