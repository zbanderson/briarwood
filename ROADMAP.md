...
**Cycle 1a outcome (2026-04-30 — LANDED).** Briarwood is hosted and accessible from phone+laptop on default subdomains. **API:** `https://briarwood.fly.dev` (Fly app `briarwood`, region `ewr`, `shared-cpu-1x` 1 GB, always-on, 1 GB encrypted volume `briarwood_data` mounted at `/app/data` with daily snapshots). **Web:** `https://briarwood-ai.vercel.app` (Vercel project `briarwood-ai`, root directory `web`, framework Next.js, pnpm, `maxDuration 300` on the SSE chat route). No custom domain (locked decision per launch prompt); no auth (Sunday scaffold; Vercel/Cloudflare Access SSO comes before user demo). Smoke tests post-deploy: `/healthz` 200; `/api/conversations` 200 `[]`; SEARCH turn ("what is for sale in Belmar, NJ") streams 8 listings + 6 map pins through `listing_discovery` + `geocoder` with no errors; BROWSE turn (pinned `1008 14th Ave Belmar`) streams 194 SSE events through 8 modules (`comp_set`, `value_thesis`, `confidence`, `strategy_fit`, `rent_outlook`, `visualizer`, `listing_discovery`, `geocoder`) including `comps_preview`, `value_thesis`, `strategy_path`, `rent_outlook`, `trust_summary`, `verifier_report`, 4 chart events, full three-section newspaper hierarchy. **Mid-flight fixes that landed against the original Stream 2 drafts:**
1. **Volume-mask-seed-data fix** — original drafts `COPY`'d seed data into `/app/data/*` directly, but the Fly volume mount at `/app/data` masks anything baked into that path. Moved seed `COPY` targets to `/opt/seed/*` (outside the mount) and added [docker-entrypoint.sh](docker-entrypoint.sh) which no-clobber-copies seed data onto the mounted volume on every boot. First boot populates a fresh volume; subsequent boots leave volume contents untouched.
2. **`requirements.txt` was missing 3 runtime deps** (`pandas`, `requests`, `python-dotenv`) — landed locally in dev-venv but not pinned. Added (commit `dfe9e71`).
3. **5 small seed dirs were excluded from the image** (`market_history`, `listing_index`, `model_quality`, `outcomes`, `manual_entries`, total ~45 KB) — `.dockerignore` was overly aggressive. Added to the COPY + entrypoint list and removed from `.dockerignore` (commit `a5ddb52`).
4. **Owner-side blip:** Fly's "Launch from GitHub" flow auto-generated a phantom app name `app-blue-forest-4682` and wired GitHub-triggered auto-deploy to it. Owner created `briarwood` separately; the auto-deploy keeps failing harmlessly to the phantom app name. Filed as a §4 cleanup (disconnect the GitHub integration in the Fly dashboard or update the workflow to target `briarwood`); not a launch blocker.

**Owner-side payment / quota events:** Fly account + Vercel account created Sunday; payment-on-file cleared without anti-fraud holds; ATTOM API key returned 401 Unauthorized partway through Cycle 1b (server-side persistent block; quota or revocation TBD per the §4 ATTOM backfill follow-up entry). Pre-deploy secret rotation: 7 keys (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `ATTOM_API_KEY`, `SEARCHAPI_API_KEY`, `GOOGLE_MAPS_API_KEY`, `MAPBOX_TOKEN`, `TAVILY_API_KEY`) issued as Fly secrets via `fly secrets set --app briarwood`; `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY`, `NEXT_PUBLIC_MAPBOX_TOKEN`, `BRIARWOOD_API_URL=https://briarwood.fly.dev` set on Vercel and rebuilt without cache to bake into the client bundle.
