...
8. React components in [web/src/components/chat/](web/src/components/chat/) render verdict cards, comp previews, risk profiles, scenario tables, maps, and charts. **Phase 4c (closed 2026-04-29)** added tier-aware rendering: when `ChatMessage.answerType === "browse"` (sourced from the `message` SSE event's `answer_type` field, persisted on `messages.answer_type`), `AssistantMessage` renders a three-section newspaper-hierarchy layout — `BrowseRead` ([browse-read.tsx](web/src/components/chat/browse-read.tsx)) + `BrowseScout` ([browse-scout.tsx](web/src/components/chat/browse-scout.tsx)) + `BrowseDeeperRead` ([browse-deeper-read.tsx](web/src/components/chat/browse-deeper-read.tsx)) — instead of the legacy card stack. Sections share a single primitive at [browse-section.tsx](web/src/components/chat/browse-section.tsx) (small-caps section label + 1px top rule + 2rem padding; no nested boxed cards). Section A (`BrowseRead`) renders stance pill + ask/fair-value headline + masthead `market_trend` chart + flowed synthesizer prose. Section B (`BrowseScout`) renders the playful Scout treatment with the existing `ScoutFinds` cards as a peer section. Section C (`BrowseDeeperRead`) renders eight chevron-list drilldowns over the [browse-drilldown.tsx](web/src/components/chat/browse-drilldown.tsx) primitive — Comps / Value thesis / Projection / Rent / Town context / Risk / Confidence & data / Recommended path — each with a data-derived italic teaser hook below the closed-row label and a `framed={false}` body that drops the boxed-card chrome. All other tiers (DECISION / EDGE / PROJECTION / RISK / STRATEGY / RENT_LOOKUP) continue to render the legacy card stack unchanged. Phase 4c Cycle 5 produced a chart-library evaluation memo at [`docs/CHART_LIBRARY_EVAL_2026-04-29.md`](docs/CHART_LIBRARY_EVAL_2026-04-29.md); owner picked Apache ECharts and the chart-renderer migration ([`CHART_MIGRATION_HANDOFF_PLAN.md`](CHART_MIGRATION_HANDOFF_PLAN.md), ROADMAP §3.6) closed 2026-04-30 — all eight chart kinds render through ECharts via a single `next/dynamic({ ssr: false })` boundary; eval sandbox + Recharts/Nivo deps deleted in Cycle 3.
...
| `chart-frame.tsx` + `chart-echarts.tsx` | `chart-frame.tsx` owns the `<figure>` chrome (title, subtitle, provenance chips, `why_this_chart` prose, MetricChip rows, footer notes) and lazy-imports the chart body. All eight `ChartSpec` kinds (scenario_fan, cma_positioning, risk_bar, rent_burn, rent_ramp, value_opportunity, horizontal_bar_with_ranges, market_trend) render through [`chart-echarts.tsx`](web/src/components/chat/chart-echarts.tsx) — a single `next/dynamic({ ssr: false })`-boundaried Apache ECharts module that switches on `spec.kind` to one of eight option-builders. Phase 3 Cycle A retired the iframe-Plotly path; Apache ECharts migration ([`CHART_MIGRATION_HANDOFF_PLAN.md`](CHART_MIGRATION_HANDOFF_PLAN.md), ROADMAP §3.6) closed 2026-04-30. ECharts engine chunk (~366 KB gz) loads only when chart events render; non-chart routes carry zero ECharts cost in first-load chunks. Theme colors resolve through [`web/src/lib/chat/chart-tokens.ts`](web/src/lib/chat/chart-tokens.ts)'s `getChartTokens()` helper. |
